...
import { DemographicData, getDemographicFields, loadDemographicData, mapYearToAgeGroup, saveDemographicData } from
'@/lib/demographics';
...
const [demographicData, setDemographicData] = useState<DemographicData>(loadDemographicData());
    const [voteData, setVoteData] = useState<{ voteId: number; votePosition: string } | null>(null);
        ...
        const form = useForm<z.infer<typeof FormSchema>>({
            ...
            (field) => demographicData[field.key as keyof DemographicData] && demographicData[field.key as keyof
            DemographicData] !== '',
            );
                ...
                <DialogContent className="sm:max-w-[500px]">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-left">
                            Zur Abstimmung sind demografische Angaben erforderlich. Diese werden anonymisiert
                            verarbeitet. Details finden Sie in unserer
                            <Link href="/datenschutz" className="text-blue-500"> Datenschutzerklärung</Link>.
                        </DialogDescription>
                    </DialogHeader>

                    <ScrollArea className="h-[50vh] max-h-[50vh] relative">
                        <div className="grid gap-4">
                            {getDemographicFields().map((field) => (
                            <div key={field.key} className="grid gap-2">
                                <Label htmlFor={field.key}>{field.label}</Label>
                                <Select value={demographicData[field.key as keyof DemographicData]}
                        ...
                        <Form {...form}>
                            <FormField control={form.control} name="saveDataLocally" render={({ field })=> (
                                <FormItem
                                    className="mt-4 mb-14 flex flex-row items-start space-y-0 space-x-3 rounded-md border p-4">
                                    <FormControl>
                                        <Checkbox id="save-data" checked={field.value}
                                            onCheckedChange={field.onChange} />
                                    </FormControl>
                                    <div className="space-y-1 leading-none">
                                        <FormLabel htmlFor="save-data">
                                            Daten für zukünftige Abstimmungen lokal in diesem Browser speichern
                                        </FormLabel>
                                    </div>
                                </FormItem>
                                )}
